...
public class Method03Main {
	...
	public static void main(String[] args) {
        ...
        System.out.println(CalcTotal(korean, english, math));
 
		// calcAvg() 메소드를 정의+호출 하여 평균 계산하고 출력
		System.out.println(CalcAvg(CalcTotal(korean, english, math)));

		// calcGrade() 메소드를 정의+호출 하여 학점(A, B, C, D, F)을 출력
		// 평균 90 이상이면 A, 80 이상이면 B, 70 이상이면 C, 60 이상이면 D
		// 나머지는 F
		System.out.println(calcGrade(CalcAvg(CalcTotal(korean, english, math))));
		System.out.println("\n프로그램 종료");
	} // end main()
