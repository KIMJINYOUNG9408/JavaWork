...
public class File16Main {
	...
	public static void main(String[] args) {
		...
		// 폴더 생성: mkdir()
		File f = new File(path); // File 객체 생성. --> 디스크에 물리적인 파일 / 디렉토리가 만들어진거은 아니다.
		...
		System.out.println();
		// 파일 생성 : createNewFile()
		File f2 = new File(f, TEST_FILE); // TEST_FILE = > dummy.txt
											// "test/dummy.txt" , File(디렉토리 File, 파일명)
		System.out.println(f2.getAbsolutePath()); // 절대 경로

		if (!f2.canExecute()) { // 파일이 존재하는지 체크
			// 파일에 존재하지 않으면 생성
			try {
				if (f2.createNewFile()) {
		...
		File f3 = new File(f, TEST_RENAME); // re_dummy.txt f는 TEST폴더 표현
		System.out.println(f3.getAbsolutePath()); // 절대 경로 확인.

		System.out.println();
		if (f2.canExecute()) { // test/dummy.txt 가 존재하는지 체크
			// 변경
			if (f2.renameTo(f3)) {
