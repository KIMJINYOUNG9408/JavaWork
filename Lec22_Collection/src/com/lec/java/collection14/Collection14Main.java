...
public class Collection14Main {
	...
	public static void main(String[] args) throws ParseException {
		...
		List<String> list1 = new LinkedList<String>();   // 다형성으로 가능한 부분.
		...
		Collections.sort(list3); // Student에 Comparable<> 구현 안되어 있으면 에러
		// Comparable<> 구현후 실행
		System.out.println(list3);
		...
		// comparable 하고 추가로 필요한건 comparator !!
		...
		Collections.shuffle(arrList); // 섞기
		arrList = arrList.subList(0,3); // 3개만 짜르기
		arrList.toArray(arr); // 다시 배열로 보내기
		...
		System.out.println(Collections.max(list3)); // 내림차순 되어있어서 80점이 max
		System.out.println(Collections.min(list3)); // 내림차순 되어있어서 30점이 min
...
class Student implements Comparable<Student> { // Student객체를 sort하기 위해서 해주어야함.
	String name;
	double point;
	...
	@Override
	public int compareTo(Student o) { // 메소드 오버라이딩도 해줘야 함.
...
//Sutdent를 이름(name) 으로 정렬 해줄 수 있는 Comparator<> 제공
class Asc implements Comparator<Student>{
	...
	public int compare(Student o1, Student o2) {
		...
		return  o1.name.compareTo(o2.name); // 문자열은 바로 compareTo 가능
	}
...
// Student 를 이름 내림차순 정렬해줄수 있는 Comparactor<> 제공
class Desc implements Comparator<Student>{
	...
	public int compare(Student o1, Student o2) {
		...
		return  -(o1.name.compareTo(o2.name)); // return o2.name.compareTo(o1.name));
	}
