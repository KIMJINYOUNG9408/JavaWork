...
//			app.download2(dataUrls[i]);
	...
	public void download1(String srcUrl) throws IOException {
		...
		url = new URL(srcUrl);
		in = url.openStream(); // URL -> InputStream 뽑아주는것
	...
	public void download2(String srcUrl) throws IOException {
		...
		String raw = conn.getHeaderField("Content-Disposition"); // 다운로드 받은 파일 이름
		...
		byte[] buf = new byte[contentLength]; // 다우로드 받을 크기만큼의 버퍼 준비
		int byteImg;
		...
		System.out.println("\t" + contentLength + "bytes " + contentType + "    " + raw);
	} // end download2
