...
//			System.out.println(cnt + "개 row(행)이 INSERT 됨");
			...
			while(rs.next()) { // next() 레코드 하나 추출하고 true 리턴 , 더이상 뽑아낼 레코드 없으면 false 리턴
				...
				int no = rs.getInt("mb_no"); // getXXX() 에 인덱스 명시 1부터 시작
				// mb_no 컬럼값이 null이면 0 리턴
				String name = rs.getString("mb_name");
				String birthdate = "";
				
				// java.sql.Date, java.sql.Time
				Date d = rs.getDate("mb_birthdate"); // 날짜만 가져옵니다. -> Java.sql
			...
			try { // 나중에 만들어진 인스턴스부터 먼저 close() 해주자.
				if(rs != null) rs.close();
