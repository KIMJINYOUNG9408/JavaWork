...
public class For01Main {
	...
	public static void main(String[] args) {
		...
		// 지역변수 : local variable
		// 블럭 { .. } 안에서 선언한 변수는
		// 선어난 이후 그 블럭안에서만 사용 가능.
		// 블럭이 끝나면 소멸됨.
		for(int k = 1; k % 27 != 0; k+=4) {
			System.out.println(k);
		}
//		System.out.println("for 종료후 k = " + k);  // 지역변수 범위를 벗어낫기 때문에
		
		{
			int a= 100;
			System.out.println("a = " + a);
			{
				// int a = 200;  안쪽 블록에서는 바깥족 블록을 기본적으로 사용 가능 하기 때문에.
				                 // 안쪽의 블럭은 바깥쪽 블럭의 지역변수 사용 가능
				                 // 바깥블럭 지역변수와 동일 이름의 변수 사용 불가
				int b = 200;
				System.out.println("b = " + b);
			}
//			System.out.println("b = " + b); // b가 소멸되고 사용했기때문에 error
		}
