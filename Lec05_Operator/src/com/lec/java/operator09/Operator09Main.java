...
public class Operator09Main {
	...
	public static void main(String[] args) {
		...
		byte b1 = 10, b2 = 20;
      //byte b3 = b1 + b2; // byte + byte -> int 타입으로 결과를 담는다. 
		
		short s1 = 10, s2 = 20;
	  //short s3 = s1 + s2; // short + short -> int 타입으로 결과를 담는다.
	  //short s3 = b1 + s1  // byte + short -> int 타입으로 결과를 담는다.
		
		char ch1 = 10, ch2 = 20;
	  //char ch3 = ch1 + ch2; // char + char -> int 타입으로 결과를 담는다.
	    char ch3 = (char)(ch1 + ch2); // 명시적 형변환
	    
		long l1 = 10L;
	  //int n4 = n1 + l1; // int + long -> long 타입으로 결과를 담는다.
		...
		l2 = 3000 * 3000 * 3000; // int 타입 끼리의 곱이므로 long타입이 안나옴.
		System.out.println("l2 : " + l2);
		
		// 자바의 연산결과 타입규칙과
		// 기본데이터 타입의 한계값 인지하고 있어야 함.
		// 생각지도 못한 결과가 나올수도 있기 때문에.
