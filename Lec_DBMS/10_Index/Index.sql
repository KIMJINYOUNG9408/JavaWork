...
--사원의 인덱스
SELECT empno, ROWID FROM T_EMP  -- ROWID 는 고유의 인덱스 ROWID, ROW INSERT 될때마다 부여된다, SELECT시 기본으로 참조 
ORDER BY empno DESC; 

SELECT ename, ROWID FROM t_emp ORDER BY ename DESC; 
-- 인덱스 <key, rowid> 에 입력
-----------------------------------------------------------
-- (1) B-tree 인덱스
--	① UNIQUE INDEX
...
CREATE UNIQUE INDEX idx_dept_dname ON t_dept2(dname);

INSERT INTO t_dept2 VALUES(9100, '임시매장', 1006, '서울지사');

INSERT INTO t_dept2 VALUES(9101, '임시매장', 1006, '부산지사'); 
--SQL Error [1] [23000]: ORA-00001: unique constraint (SCOTT0316.IDX_DEPT_DNAME) violated
