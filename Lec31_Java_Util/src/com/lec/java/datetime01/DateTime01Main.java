...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception {
		...
		int year = now.getYear() + 1900; // 연도는 1900 이후 경과 년도 +1900 필요 // 짝대기 그어진거는 앞으로 이걸 안쓸겁니다 .
		int month = now.getMonth() + 1; // 월은 0 기준
		int day = now.getDate(); // 일
		int week = now.getDay(); // 요일 == > 일요일 = 0
		int hour = now.getHours();
		int minute = now.getMinutes();
		int second = now.getSeconds();

		System.out.println(year + "년 " + month + "월 " + day + "일 " + hour + "시 " + minute + "분 " + second + "초 입니다");
		System.out.println();
		...
		long baseTime = System.currentTimeMillis(); // 현재 시간을 ms 로 리턴
		...
		String time = String.format("0%2d:%02d:%02d.%03d",hr,min,sec,millis);
