...
//				responseXML(request, response); // jdom 사용 . 
	...
	private void responseJSON(HttpServletRequest request, HttpServletResponse response) { // 첫버전
	...
	private void responseXML(HttpServletRequest request, HttpServletResponse response) { // 첫버전
		WriteDTO[] dtoArr = (WriteDTO[]) request.getAttribute("list");
		...
		if (dtoArr == null) {
			...
			// 데이터 개수
			int count = dtoArr.length;
			Element countElement = new Element("Count").setText("" + count).setAttribute("id", "ccc").setAttribute("pw",
					"xxx");
